...
class Restorer(object):
    ...
    def setup_model(self):
        ...
        if self.config.lan_model:
            if self.config.model_name == "MEGA_HP":
    ...
    def train(self):
        ...
        while not self.finished:
            ...
            for data in trainset_generator:
                ...
                ys_= self.model(xs, x_masks,ys, y_masks,device = self.config.device)

                if self.config.use_rdrop:
                    ys_2 = self.model(xs, x_masks,ys, y_masks,device = self.config.device)
                ...
                if (train_iteration + 1) % self.config.gradient_accumulation_steps == 0:
                    self.opt.step()
                    self.opt.zero_grad()
    ...
    def validate(self):
        ...
        with torch.no_grad():
            for data in validset_generator:
                ...
                train_data = (torch.LongTensor(i).to(self.config.device) for i in train_data)
                xs, x_masks, ys, y_masks = train_data
                ys_ = self.model(xs, x_masks,device = self.config.device)
                loss = self.criterion(ys_.view(-1, ys_.shape[-1]), ys.view(-1))
                if self.config.mask_loss:
                    loss = (loss * y_masks.view(-1)).sum() / y_masks.sum()
                else:
    ...
    def ref_test(self):
        ...
        if self.max_avg_f1 <= eva_matrix.avg_f1:
            self.max_avg_f1 = eva_matrix.avg_f1
            # save model
            pipeline.save_model(
                self.step, self.epoch, self.model.state_dict, self.config.SAVE_POINT)
