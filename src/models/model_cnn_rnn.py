...
class BertChineseLinearPunc(nn.Module):
    def __init__(self, segment_size, output_size, dropout, vocab_size):
        ...
        print("fucking code**************")
        self.bert = AutoModel.from_pretrained(pretrained_model_name_or_path='bert-base-chinese')
        # self.bert_vocab_size = vocab_size
        # self.bn = nn.BatchNorm1d(segment_size*self.bert_vocab_size)
        # self.fc = nn.Linear(segment_size*self.bert_vocab_size, output_size)
        self.bert_size = 768
...
class BertChineseSlimCNNBert(nn.Module):
    def __init__(self, segment_size, output_size, dropout, vocab_size):
        ...
        # self.bert_2 = BertModel.from_pretrained('bert_base_chinese')
        # self.bert_vocab_size = vocab_size
        # self.bn = nn.BatchNorm1d(segment_size*self.bert_vocab_size)
        # self.fc = nn.Linear(segment_size*self.bert_vocab_size, output_size)
        self.bert_size = 768
...
class BertChineseEmbSlimCNNlstmBert(nn.Module):
    ...
    def __init__(self, segment_size, output_size, dropout, vocab_size):
        ...
        self.bert_2 = BertModel.from_pretrained(pretrained_model_name_or_path='bert-base-chinese')
        ...
        self.conv = nn.ModuleList()
        cnn_kernel_size = (3, self.bert_size) # 卷积核大小
        cnn_filter_num = self.bert_size # 卷积核个数 
        cnn_layer_num = 5 
        self.cnn_kernel_size = cnn_kernel_size
        self.cnn_filter_num = cnn_filter_num
        self.cnn_layer_num = cnn_layer_num
        ...
        for i in range(cnn_layer_num):
            module_tmp = nn.ModuleDict({
                'conv_w_{}'.format(i):
                nn.Conv2d(1,     # 输入通道            
                          self.cnn_filter_num, # 输出通道 就是卷积核个数
                          self.cnn_kernel_size,
                          padding=((cnn_kernel_size[0] - 1) // 2,
                                   0)), # 保证每个词embedding都能参与卷积
                'conv_v_{}'.format(i):
                nn.Conv2d(1,
                          self.cnn_filter_num,
                          self.cnn_kernel_size,
        ...
        self.dropout = nn.Dropout(dropout)
        # lstm hidden*2
        self.fc_size = self.bert_size+self.bert_size*2  # 为什么是3倍? 
        self.fc = nn.Linear(self.fc_size, output_size)

    def forward(self, x):
        emb2 = self.bert_2(x)[0] # 获取bert的输出 [10,200,768]
        ...
        if attention_mask is None: # attention的mask矩阵初始化为 1
            attention_mask = torch.ones(input_shape, device=device)
        if token_type_ids is None: # token的id初始化为0
            token_type_ids = torch.zeros(input_shape, dtype=torch.long, device=device)
        ...
        # print()
        for i, conv_dict in enumerate(self.conv):
            w = conv_dict['conv_w_{}'.format(i)](cnn_out) # [10,768,200,1]
            v = conv_dict['conv_v_{}'.format(i)](cnn_out)
            cnn_out = w * torch.sigmoid(v)
            # 有bert_size个filter，拼成原来的size
            cnn_out = torch.cat([cnn_out[:, i_tmp, :, :] for i_tmp in range(self.cnn_filter_num)], dim=-1) # [10,200,768]
            cnn_out = cnn_out.unsqueeze(1)
            cnn_out = skip_connection + cnn_out  # 类似残差连接
            skip_connection = cnn_out

        # ******************cnn*****end***********
        output1 = cnn_out.squeeze(1) # [10,200,768]

        output1, _ = self.lstm_cnn(output1) # 这是词级别的embedding  [10,200,1536]

        # bert_2
        output2 = emb2  # 字符级别的embedding
